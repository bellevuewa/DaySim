  ...
  public sealed class ChoiceModelRunner : IChoiceModelRunner {
    ...
    private static void RunPartialJointHalfTourModelSuite(HouseholdDayWrapper householdDay, IPartialHalfTourWrapper partialJointHalfTour) {
      ...
      for (int i = 2; i <= partialJointHalfTour.Participants; i++) {
        ...
        double circuityDistance = (Global.Configuration.UseShortDistanceNodeToNodeMeasures || Global.Configuration.UseShortDistanceCircuityMeasures) ? tour[i - 1].DestinationParcel.CalculateShortDistance(tour[i].DestinationParcel, /* doNewCorrections */ false) : Constants.DEFAULT_VALUE;
